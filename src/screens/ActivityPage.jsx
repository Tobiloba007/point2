...
export default function ActivityPage() {
  ...
  return (
    <SafeAreaView style={{ flex: 1, paddingTop: insets.top }}>
      <View
        className={`flex-1 items-center justify-start w-full bg-white pt-4`}
      >
        <Text className={`text-lg text-[#344054] font-['bold']`}>Activity</Text>

        <View
          className={`flex flex-row items-center justify-start w-full mt-5 mb-3 shadow-2xl`}
        >
          <ScrollView
            horizontal={true}
             ...
             px-2 rounded-full mx-[7px] ${
               active === item.id ? "opacity-100" : "opacity-40"
                ...
                >
                  ...
                  <View
                    className={`flex items-center justify-center ${item.quantityColor} h-5 px-2 rounded-full ml-[6px]`}
                  >
                    <Text
                      className={`text-xs text-[${item.textColor}] font-['bold']`}
                    >
                      {item.quantity}
                    </Text>
                  </View>
